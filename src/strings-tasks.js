...
function isString(val) {
  if (val === null || val === undefined || typeof val.valueOf() !== 'string') {
    return false;
  }
  return true;
}
...
function concatenateStrings(val1, val2) {
  const res = String.prototype.concat(val1, val2);
  return res;
}
...
function formatTime(min, sec) {
  const a = min.toString().padStart(2, '0');
  const b = sec.toString().padStart(2, '0');
  return `${a}:${b}`;
}
...
function isPalindrome(str) {
  const initial = str.replace(/[^A-Z]/gi, '').toLowerCase();
...
function reverseWords(str) {
  ...
  const reversed = [];
  words.forEach((char) => {
    reversed.push(char.split('').reverse().join(''));
  });
...
function encodeToRot13(str) {
  ...
  return str.replace(/[A-Za-z]/g, (char) => cba[abc.indexOf(char)]);
}
